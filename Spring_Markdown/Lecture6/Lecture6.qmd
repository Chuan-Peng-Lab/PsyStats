...
疑问： 1. 伯努利试验中的重复次数和 b(k; n,p)中的关系; 2. 贝叶斯公式中的似然和上节课提到的似然之间的关系; 3. 似然在何种情况下和概率是等价的

## 伯努利试验中的重复次数和 b(k; n,p)中的关系

-   重复次数$\ne$ b(k; n,p)中的n
-   b(k; n,p)是一个理想的概率模型，不受现实发生的次数的影响
...
## 贝叶斯公式中的似然和上节课提到的似然之间的关系

$$p(\theta|X) = \frac{P(X|\theta)p(\theta)}{p(X)}$$ 似然是贝叶斯学派和频率学派共有的概念，上节课更多地是从频率学派的视角来看待这一概念。频率学派认为概率是客观的，世界存在一个真值，该值可以通过最大似然法来估计出来。而在贝叶斯学派的观念里，概率是主观的，事件发生的可能性不仅取决于我们观察到的证据，即似然，还取决于我们对该事件本身的先验观念。

似然在两者之间的差别仅体现在理念上，在数学上是等价的。

## 似然在何种情况下和概率是等价的

![Image Name](https://cdn.kesci.com/upload/image/rifpj0n6y4.png?imageView2/0/w/960/h/960){fig-align="center"}

- 图中可以看出为何似然之和不为1而概率之和为1，似然之和取决于$\theta$的参数空间
...
如果有 n 排钉子，n+1个槽，钉子每次下落都有两种可能的方向，则它落入第k个槽的概率。
...
<center>直观图像上，二项分布的极限分布（$n \rightarrow \infty$）是一种对称钟形分布，那么真实数据上也是如此吗？</center>
...
n增大,平均值趋近于$\mu = np$, 标准差趋近于$\sigma = \sqrt(npq)$
...
掷1000次硬币，硬币正面朝上次数为500 ～ 510次时的概率：
...
## 连续变量的概率密度函数(Probability density function, PDF)
...
在概率质量函数中，某事件发生的概率表示为该事件对应的y值之和 在概率质量函数中，某事件发生的概率表示为该事件对应的曲线下面积
...
-   最开始中心位置是真值μ (e.g., 0)，每遇到一个障碍，钉子会完全随机向左或右改变方向，将这个转向当作随机误差
