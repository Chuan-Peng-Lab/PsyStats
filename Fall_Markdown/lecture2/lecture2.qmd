...
$$F=\frac{MS_B}{MS_W}$$ </br> 当H~0~为真时，*F*值服从一个广为人知的概率分布。为了纪念R.A.Fisher，这个分布称为*F*分布。</br>
...
library(pacman)
library(apaTables)
p_load('tidyverse','bruceR','patchwork','ggdist','stats')
```
...
#进行双因素方差分析
results=MANOVA(data=data, dv="variable", between=c("region"))
results[1]
#data$region <- factor(data$region, levels = c("1", "2", "3"))
#model <- aov(variable ~ region, data = data)
...
1.  因素指实验中的自变量。实验中包括一个自变量时，为单因素设计；包含两个自变量时为二因素设计。</br></br>

2.  一个因素的不同情况，称为这一因素的不同水平。2×3表示存在两个因素，第一个因素有2个水平，第二个因素有3个水平。</br></br>
