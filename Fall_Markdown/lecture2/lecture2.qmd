...
假定有三组数据，每组数据包含30个值，都来自平均值为100，标准差为15的正态分布，对其进行F检验并重复该过程1000次后，p值\<=0.05的情况如下：
...
假定有三组数据，每组数据包含30个值，都来自平均值为100，标准差为15的正态分布，对其两两之间进行t检验并重复该过程1000次后，每次检验p值\<=0.05的情况如下：
...
```{r}
p1<-ggplot(sim2, aes(x=iter,
                 y=t_val1))+
  geom_line()+
  theme_bw()

p2<-ggplot(sim2, aes(x=iter,
                 y=p_val1))+
  geom_line()+
  geom_hline(yintercept = 0.05, linetype = "dashed")+
  theme_bw()
p1+p2
print(paste('第一次比较，p值<=0.05的占比为',nrow(sim2[sim2$p_val1<=0.05,])/nrow(sim2)))
```
...
p2<-ggplot(sim2, aes(x=iter,
  ...
  theme_bw()
p1+p2
print(paste('第二次比较，p值<=0.05的占比为',nrow(sim2[sim2$p_val2<=0.05,])/nrow(sim2)))
```
...
p2<-ggplot(sim2, aes(x=iter,
  ...
  theme_bw()
p1+p2
print(paste('第三次比较，p值<=0.05的占比为',nrow(sim2[sim2$p_val3<=0.05,])/nrow(sim2)))
```

## 

```{r}
print(paste('三次比较中，至少有一次p值<=0.05的占比为',nrow(sim2[sim2$p_val3<=0.05|sim2$p_val2<=0.05|sim2$p_val1<=0.05,])/nrow(sim2)))
```
