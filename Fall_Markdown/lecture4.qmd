...
$$SS_{总变异}=SS_{与处理无关的个体差异(被试间)}+SS_{被试内}$$ 
$$SS_{被试内}=SS_A+SS_{A*被试}+SS_B+SS_{B*被试}+SS_{AB}+SS_{AB*被试}$$
...
$$SS_总=\sum{X^2}-\frac{(\sum{X})^2}{npq}$$
其中，X是单个被试的观测值，n是每个实验处理中的被试数量，p是A因素的水平数，q是B因素的水平数

$$SS_{被试间}=\frac{(\sum{S})^2}{pq}-\frac{(\sum{X})^2}{npq}$$
其中，S是单个被试在所有实验处理观测值的总和
...
$$SS_A=\frac{(\sum{A})^2}{nq}-\frac{(\sum{X})^2}{npq}$$
其中，A是A因素每个水平下所有被试观测值的总和

$$SS_B=\frac{(\sum{B})^2}{np}-\frac{(\sum{X})^2}{npq}$$
其中，B是B因素每个水平下所有被试观测值的总和

$$SS_{AB}=\frac{(\sum{AB})^2}{n}-\frac{(\sum{X})^2}{npq}-SS_A-SS_B$$
其中，AB是某个实验处理结合下所有被试观测值的总和
...
$$SS_{A*被试}=\frac{(\sum{AS})^2}{q}-\frac{(\sum{X})^2}{npq}-SS_{被试间}-SS_A$$
其中，AS是接受A因素某水平的某个被试的观测值

$$SS_{B*被试}=\frac{(\sum{BS})^2}{p}-\frac{(\sum{X})^2}{npq}-SS_{被试间}-SS_B$$
其中，BS是接受B因素某水平的某个被试的观测值
...
SS_A_被试 <- ((sum(a1^2)+sum(a2^2))/ q) - (sum(X)^2) / (n * p * q)-SS_between_subject-SS_A
SS_A_被试
```
```{r,echo=TRUE}
#提取所需数据
...
SS_B_被试 <- ((sum(b1^2)+sum(b2^2)+sum(b3^2))/ p) - (sum(X)^2) / (n * p * q)-SS_between_subject-SS_B
SS_B_被试
```
```{r,echo=TRUE}
SS_A_B_被试=SS_within_subject-SS_A-SS_B-SS_AB-SS_A_被试-SS_B_被试
SS_A_B_被试
...
$$MS_{A*被试}=\frac{SS_{A*被试}}{df_{A*被试}}$$
$$MS_{B}=\frac{SS_{B}}{df_{B}}$$
$$MS_{B*被试}==\frac{SS_{B*被试}}{df_{B*被试}}$$
$$MS_{A*B}=\frac{SS_{A*B}}{df_{A*B}}$$
$$MS_{A*B*被试}=\frac{SS_{A*B*被试}}{df_{A*B*被试}}$$
```{r,echo=TRUE}
MS_A=SS_A/df_A
MS_A
```
```{r,echo=TRUE}
MS_A_被试=SS_A_被试/df_A_被试
MS_A_被试
```
...
```
```{r,echo=TRUE}
MS_A_B_被试=SS_A_B_被试/df_A_B_被试
MS_A_B_被试
```
## 第五步：计算F值

$$F_A=\frac{MS_A}{MS_{A*被试}}$$
$$F_B=\frac{MS_B}{MS_{B*被试}}$$
$$F_{AB}=\frac{MS_{AB}}{MS_{AB*被试}}$$
...
## 第五步：计算均方
$$MS_A=\frac{SS_A}{df_A}$$
$$MS_{被试A}=\frac{SS_{被试A}}{df_{被试A}}$$
$$MS_B=\frac{SS_B}{df_B}$$
$$MS_{AB}=\frac{SS_{AB}}{df_{AB}}$$
$$MS_{B*被试A}=\frac{SS_{B*被试A}}{df_{B*被试A}}$$
---
## 第六步：计算*F*值

$$F_A=\frac{MS_A}{MS_{被试A}}$$
$$F_B=\frac{MS_B}{MS_{B*被试A}}$$
$$F_{AB}=\frac{MS_{AB}}{MS_{B*被试A}}$$
